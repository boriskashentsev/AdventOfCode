    ...
    for i in range(len(wire)):
        ...
        if (side[0] == 'R'):
            for j in range(length):
                loc[0] += 1
                if (j == length - 1 and i != len(wire) - 1):
                    field[loc[1]][loc[0]] = '+'
                else:
                    field[loc[1]][loc[0]] = '-'
        elif (side[0] == 'L'):
...
def doSteps(stepMatrix, wire, zero):
    loc = zero[:]
    number = 0
    for i in range(len(wire)):
        side = wire[i]
        ...
        if (side[0] == 'R'):
            for j in range(length):
                loc[0] += 1
                stepMatrix[loc[1]][loc[0]] = stepMatrix[loc[1]][loc[0] - 1] + 1
        elif (side[0] == 'L'):
            for j in range(length):
                loc[0] -= 1
                stepMatrix[loc[1]][loc[0]] = stepMatrix[loc[1]][loc[0] + 1] + 1
            ...
            for j in range(length):
                loc[1] += 1
                stepMatrix[loc[1]][loc[0]] = stepMatrix[loc[1] - 1][loc[0]] + 1
        elif (side[0] == 'D'):
            for j in range(length):
                loc[1] -= 1
                stepMatrix[loc[1]][loc[0]] = stepMatrix[loc[1] + 1][loc[0]] + 1
...
dim1 = minmax(wire1.split(','))
dim2 = minmax(wire2.split(','))

# field1 = [['.' for x in range(max([dim1[0][1],dim2[0][1]])-min([dim1[0][0],dim2[0][0]]) + 1)] for y in range(max([dim1[1][1],dim2[1][1]])-min([dim1[1][0],dim2[1][0]]) + 1)]
# field2 = [['.' for x in range(max([dim1[0][1],dim2[0][1]])-min([dim1[0][0],dim2[0][0]]) + 1)] for y in range(max([dim1[1][1],dim2[1][1]])-min([dim1[1][0],dim2[1][0]]) + 1)]

step1 = [[-1 for x in range(max([dim1[0][1],dim2[0][1]])-min([dim1[0][0],dim2[0][0]]) + 1)] for y in range(max([dim1[1][1],dim2[1][1]])-min([dim1[1][0],dim2[1][0]]) + 1)]
step2 = [[-1 for x in range(max([dim1[0][1],dim2[0][1]])-min([dim1[0][0],dim2[0][0]]) + 1)] for y in range(max([dim1[1][1],dim2[1][1]])-min([dim1[1][0],dim2[1][0]]) + 1)]

zero = [-min([dim1[0][0],dim2[0][0]]), -min([dim1[1][0],dim2[1][0]])]
# field1[zero[1]][zero[0]] = 'o'
# field2[zero[1]][zero[0]] = 'o'
step1[zero[1]][zero[0]] = 0
step2[zero[1]][zero[0]] = 0
# trace(field1, wire1.split(','), zero)
# trace(field2, wire2.split(','), zero)

doSteps(step1, wire1.split(','), zero)
doSteps(step2, wire2.split(','), zero)
# printTrace(field1)
# printTrace(field2)

# minDist = len(field1) + len(field1[0])

# for i in range (len(field1)):
#     for j in range (len(field1[0])):
#         if (field1[i][j] not in '.o' and field2[i][j] not in '.o'):
#             if(minDist > abs(zero[1]-i) + abs(zero[0]-j)):
#                 minDist = abs(zero[1]-i) + abs(zero[0]-j)
...
for i in range (len(step1)):
    for j in range (len(step1[0])):
        if (step1[i][j] > 0 and step2[i][j] > 0):
            if(minDist > step1[i][j]+step2[i][j] or minDist < 0):
                minDist = step1[i][j]+step2[i][j]
